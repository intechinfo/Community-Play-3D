...
float getRayLengthAt(vec2 coords)
{
	vec3 tLStart = mix(LineStarts0, LineStarts1, coords.x); 
	vec3 bLStart = mix(LineStarts2, LineStarts3, coords.x);
	vec3 tLEnd = mix(LineEnds0, LineEnds1, coords.x);
	vec3 bLEnd = mix(LineEnds2, LineEnds3, coords.x);

	return length(mix(tLEnd, bLEnd, coords.y) - mix(tLStart, bLStart, coords.y));
}
...
void main()
{
	...
	for(int i = 0;i < SAMPLE_COUNT;++i)
	{
		...
		sampleDepth = sampleDepth / getRayLengthAt(newWorldPos.xy) - DEPTH_ALIASING_EPISILON;
