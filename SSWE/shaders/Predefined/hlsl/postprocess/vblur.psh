...
float4 main(float2 texCoord : TEXCOORD0) : COLOR0
{
	...
	for (int i=0; i<9; i++)
		finalCol += tex2D(SceneBuffer, float2(texCoord.x, texCoord.y+(i-4)*blurSize))*weight[i];
...
float4 main7x7(float2 texCoord : TEXCOORD0) : COLOR0
{
	...
	// take the samples with the distance blurSize between them
	for (int i=0; i<7; i++)
		finalCol += tex2D(SceneBuffer, float2(texCoord.x, texCoord.y+(i-3)*blurSize))*weight[i];
