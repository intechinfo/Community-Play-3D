...
{	
	...
	loadRTTConfig("shaders/Predefined/rtt.xml");
	loadEffectConfig("shaders/Predefined/effect.xml");
...
void CPostProcessManager::render(E_POSTPROCESS_EFFECT effect)
{
	...
	{		
		...
		for (u32 i=0; i<EffectChain[effect].size(); i++)
		{
			...
			// bind input buffer 
			if( !postProcess->getRenderSource().equals_ignore_case(L""))
				postProcess->getMaterial().setTexture(0, RenderTargetMap[postProcess->getRenderSource()]);

			// bind output buffer
			if( !postProcess->getRenderTarget().equals_ignore_case(L""))
		...
		case io::EXN_ELEMENT:
			{
					...
					currentPostProcess)
				{
					...
					if(index < video::MATERIAL_MAX_TEXTURES)
					{
						...
						if(!texPath.equals_ignore_case(L""))
						...
						if(!texName.equals_ignore_case(L""))
