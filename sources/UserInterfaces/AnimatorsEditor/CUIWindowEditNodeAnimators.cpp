...
void CUIWindowEditNodeAnimators::fillMainArea() {
	...
	if(animatorSelected != NULL) {
		...
		for(u32 i = 0; i < attributes->getAttributeCount(); i++) {
			...
			if(type == 0) { // INT
				devices->getGUIEnvironment()->addStaticText((stringw(name) + L" : ").c_str(), rect<s32>(2, y, 110, y2), true, false, mainArea, -1, false);
				s32 var = attributes->getAttributeAsInt(i);
				intEditBox = devices->getGUIEnvironment()->addEditBox(stringw(var).c_str(), rect<s32>(110, y, 490, y2), true, mainArea, -1);
				y += 30;
				y2 += 30;
			}
				...
				floatEditBox = devices->getGUIEnvironment()->addEditBox(stringw(var).c_str(), rect<s32>(110, y, 490, y2), true, mainArea, -1);
				y += 30;
				y2 += 30;
			}
				...
				stringEditBox = devices->getGUIEnvironment()->addEditBox(stringw(var).c_str(), rect<s32>(110, y, 490, y2), true, mainArea, -1);
				y += 30;
				y2 += 30;
			}
			/*else if(type == 3) { // BOOL
				devices->getGUIEnvironment()->addStaticText((stringw(name) + L" : ").c_str(), rect<s32>(2, y, 110, y2), true, false, mainArea, -1, false);
				devices->getGUIEnvironment()->addEditBox(stringw(var).c_str(), rect<s32>(110, y, 490, y2), true, mainArea, -1);
				y += 30;
				y2 += 30;
			}*/
			else if(type == 4) { // ENUM
				...
				enumEditBox = devices->getGUIEnvironment()->addEditBox(stringw(var).c_str(), rect<s32>(110, y, 490, y2), true, mainArea, -1);
				y += 30;
				y2 += 30;
			}
			/*else if(type == 5) { // COLOR
				...
				f32 vary = var.Y;
				f32 varz = var.Z;
				vector3dXEditBox = devices->getGUIEnvironment()->addEditBox(stringw(varx).c_str(), rect<s32>(110, y, 230, y2), true, mainArea, -1);
				vector3dYEditBox = devices->getGUIEnvironment()->addEditBox(stringw(vary).c_str(), rect<s32>(230, y, 360, y2), true, mainArea, -1);
				vector3dZEditBox = devices->getGUIEnvironment()->addEditBox(stringw(varz).c_str(), rect<s32>(360, y, 490, y2), true, mainArea, -1);
				y += 30;
				y2 += 30;
			}
...
void CUIWindowEditNodeAnimators::saveChanges() {
	...
	if(animatorSelected != NULL) {
		...
		for(u32 i = 0; i < attributes->getAttributeCount(); i++) {
			...
			u32 type = attributes->getAttributeType(name);
			if(type == 0) { // INT
				newAttributes->addInt(name, core->getS32(stringc(intEditBox->getText()).c_str()));
			}
			else if(type == 1) { // FLOAT
				newAttributes->addFloat(name, core->getF32(stringc(floatEditBox->getText()).c_str()));
			}
			else if(type == 2) { // STRING
				newAttributes->addString(name, stringEditBox->getText());
			}
			else if(type == 3) { // BOOL
			...
			/*else if(type == 4) { // ENUM
				...
				newAttributes->addVector3d(name, vector3df(core->getF32(stringc(vector3dXEditBox->getText()).c_str()), 
										core->getF32(stringc(vector3dYEditBox->getText()).c_str()), core->getF32(stringc(vector3dZEditBox->getText()).c_str())));
			}
			else if(type == 9) { // VECTOR2D
				newAttributes->addVector2d(name, vector2df(core->getF32(stringc(vector2dXEditBox->getText()).c_str()), 
										core->getF32(stringc(vector2dYEditBox->getText()).c_str())));
			}
			else if(type == 10) { // RECT
