...
void CUIWindowAddWaterSurface::openWindow()
{
    ...
    m_selectButton = m_devices->getGUIEnvironment()->addButton(rect<s32>(220, 50, 290, 80), m_window, CXT_WINDOW_ADD_WATER_EVENTS_SELECT, L"Select",L"Select the mesh that will support the water surface");
    m_acceptButton = m_devices->getGUIEnvironment()->addButton(rect<s32>(5, 110, 70, 135), m_window, CXT_WINDOW_ADD_WATER_EVENTS_ACCEPT, L"Accept", L"Accept the current selection and create the water surface");
    m_cancelButton = m_devices->getGUIEnvironment()->addButton(rect<s32>(80, 110, 145, 135), m_window, CXT_WINDOW_ADD_WATER_EVENTS_CLOSE, L"Cancel", L"Cancel the current action and close this windows without adding anything");
}
...
bool CUIWindowAddWaterSurface::OnEvent(const SEvent &event)
{
	if(event.EventType == EET_GUI_EVENT)
	{
		if(event.GUIEvent.EventType == EGET_BUTTON_CLICKED)
		{
			...
			switch(id)
			{
				case CXT_WINDOW_ADD_WATER_EVENTS_SELECT:
					m_devices->createFileOpenDialog(L"Select base mesh");
					m_isFileDialogOpen = true;
					break;

				case CXT_WINDOW_ADD_WATER_EVENTS_ACCEPT:
				{
					...
					if(waterSurfaceNode)
					{
						...
						m_devices->getCoreData()->getObjectNodes()->push_back(waterSurfaceNode);
						m_devices->getCoreData()->getObjectPaths()->push_back(m_filePath);
				...
				case CXT_WINDOW_ADD_WATER_EVENTS_CLOSE:
					m_window->remove();
					break;
