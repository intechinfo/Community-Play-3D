...
void LuminanceGenerator::LuminanceCallback::OnSetConstants(IMaterialRendererServices* services, s32 userData) {
    ...
    services->setPixelShaderConstant(services->getPixelShaderConstantID("lumOffsets[0]"), reinterpret_cast<f32*>(&lumOffsets), 8);
    #endif
}
...
void LuminanceGenerator::DownsampleCallback::OnSetConstants(IMaterialRendererServices* services, s32 userData) {
    ...
    #else
    services->setPixelShaderConstant(services->getPixelShaderConstantID("tex0"), &texVar, 1);
    services->setPixelShaderConstant(services->getPixelShaderConstantID("dsOffsets[0]"), reinterpret_cast<f32*>(&dsOffsets), 18);
    services->setPixelShaderConstant(services->getPixelShaderConstantID("halfDestPixelSize"), &halfDestPixelSize, 1);
    #endif
}
