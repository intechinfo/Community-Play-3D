...
CUIWindowEditDOF::CUIWindowEditDOF(CDevices *_devices) {
	...
	ranges[0] = SRange(15, 99, 10);
	ranges[1] = SRange(16, 40, 100);
	ranges[2] = SRange(0, devices->getMayaCamera()->getFarValue(), 1000);
	ranges[3] = SRange(118, 1570, 1000);

	for (u32 i=0; i < 4; i++) {
		IGUIStaticText *tempText = gui->addStaticText(values[i].txt, rect<s32>(10, 30, 750, 50), true, true, window, -1, false);
	...
	if (event.EventType == EET_GUI_EVENT) {
		...
		if (event.GUIEvent.EventType == EGET_SCROLL_BAR_CHANGED) {
			if (event.GUIEvent.Caller == bars[0])
				devices->getDOF()->range = (f32)(bars[0]->getPos()/ranges[0].divide);

			if (event.GUIEvent.Caller == bars[1])
				devices->getDOF()->focus = (f32)(bars[1]->getPos()/ranges[1].divide);

			if (event.GUIEvent.Caller == bars[2]) {
				devices->getDOF()->sampleDist1 = (f32)(bars[2]->getPos()/ranges[2].divide);
				devices->getDOF()->sampleDist0 = devices->getDOF()->sampleDist1+0.002f;
			}

			if (event.GUIEvent.Caller == bars[3])
				devices->getDOF()->distanceScale = (f32)(bars[3]->getPos()/ranges[3].divide);
		}
	}
