...
namespace scene
{
	...
	//! constructor
	CTerrainSceneNode::CTerrainSceneNode(ISceneNode* parent, IrrlichtDevice* dev,
			io::IFileSystem* fs, s32 id, s32 maxLOD, E_TERRAIN_PATCH_SIZE patchSize,
			const core::vector3df& position,
			const core::vector3df& rotation,
			const core::vector3df& scale)
	: ITerrainSceneNode(parent, dev->getSceneManager(), id, position, rotation, scale),
	...
	TCoordScale1(1.0f), TCoordScale2(1.0f), SmoothFactor(0), FileSystem(fs), Device(dev)
	{
		#ifdef _DEBUG
			...
			s32 width, video::SColor vertexColor, s32 smoothFactor)
	{
		...
		// start reading
		const u32 startTime = Device->getTimer()->getTime();//os::Timer::getTime();
		...
		else
		{
			if ((filesize-file->getPos())/bytesPerPixel>width*width)
			{
				...
				Device->getLogger()->log("Error reading heightmap RAW file", "File is too small.");
				return false;
			}
		...
		for (s32 x = 0; x < TerrainData.Size; ++x)
		{
			...
			for (s32 z = 0; z < TerrainData.Size; ++z)
			{
				...
				if (failure)
				{
					...
					Device->getLogger()->log("Error reading heightmap RAW file.");
					mb->drop();
					return false;
				}
				vertex.Pos.Z = fz;
		...
		const u32 endTime = Device->getTimer()->getTime();//os::Timer::getTime();
	...
	void CTerrainSceneNode::render()
	{
		...
		if (DebugDataVisible)
		{
			...
			const u32 now = Device->getTimer()->getRealTime();//os::Timer::getRealTime();
			...
			io::SAttributeReadWriteOptions* options)
	{
		...
		if (newHeightmap.size() != 0 && newHeightmap != HeightmapFile)
		{
			...
			if (file)
			{
				...
				Device->getLogger()->log("could not open heightmap", newHeightmap.c_str());
		}
	...
	ISceneNode* CTerrainSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
	{
		...
		CTerrainSceneNode* nb = new CTerrainSceneNode(
			newParent, Device, FileSystem, ID,
