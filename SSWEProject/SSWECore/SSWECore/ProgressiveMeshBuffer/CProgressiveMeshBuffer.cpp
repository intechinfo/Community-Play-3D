...
ProgressiveMeshBuffer::ProgressiveMeshBuffer(irr::scene::IMeshBuffer* meshBuffer, SimplificationAlgorithm algorithm) {
	...
	if (meshBuffer->getVertexType() == video::EVT_STANDARD) {
		...
		for (irr::u32 i = 0; i < meshBuffer->getVertexCount(); i++) {
			vertices.push_back(new CVertex(i, verts[i].Pos, verts[i].Normal, verts[i].Color, verts[i].TCoords));
		}
	}
	// Vertex with two texture coordinates
		...
		for (irr::u32 i = 0; i < meshBuffer->getVertexCount(); i++) {
			vertices.push_back(new CVertex2TCoords(i, verts[i].Pos, verts[i].Normal, verts[i].Color, verts[i].TCoords, verts[i].TCoords2));
		}
	}
	// Vertex with a tangent and binormal vector
		...
		for (irr::u32 i = 0; i < meshBuffer->getVertexCount(); i++) {
			vertices.push_back(new CVertexTangets(i, verts[i].Pos, verts[i].Normal, verts[i].Color, verts[i].TCoords, verts[i].Tangent, verts[i].Binormal));
		}
	}
	...
	for (irr::u32 i = 0; i < meshBuffer->getIndexCount(); i += 3) {
		CTriangle* triangle = new CTriangle(vertices[indices[i]], vertices[indices[i + 1]], vertices[indices[i + 2]]);
...
const irr::core::array<CTriangle*>& ProgressiveMeshBuffer::getTriangles() const {
	return triangles;
}

const irr::core::array<CVertex*>& ProgressiveMeshBuffer::getVertices() const {
	return vertices;
}
...
IMeshBuffer* ProgressiveMeshBuffer::getCurrentMeshBuffer() {
	...
	// Vertex with two texture coordinates
		...
		for (irr::u32 i = 0; i < vertices.size(); ++i) {
			CVertex2TCoords* v = (CVertex2TCoords*) vertices[i];
			if (v->isOrphan()) {
				v->setActive(false);
			} else if (v && v->isActive()) {
				v->setId(vertexIndex); // Change ID
	...
	// Vertex with a tangent and binormal vector
		...
		for (irr::u32 i = 0; i < vertices.size(); ++i) {
			CVertexTangets* v = (CVertexTangets*) vertices[i];
			if (v->isOrphan()) {
				v->setActive(false);
			} else if (v && v->isActive()) {
				v->setId(vertexIndex); // Change ID
...
void ProgressiveMeshBuffer::calculateMelaxEdgeCost(Edge* edge) {
	...
	const irr::core::array<CTriangle*> facesEdge = edge->getFaces();
	const irr::core::array<CTriangle*> facesFrom = edge->getVertexFrom()->getFaces();

	// Use the triangle facing most away from the sides to determine our curvature term
	for (irr::u32 i = 0; i < facesFrom.size(); ++i) {
		const CTriangle* t1 = facesFrom[i];
		f32 minCurve = 1;

		for (irr::u32 i2 = 0; i2 < facesEdge.size(); ++i2) {
			const CTriangle* t2 = facesEdge[i2];
...
//	}
	...
	CVertex* v = edge->getVertexTo(); // Calculate ~v
...
f32 ProgressiveMeshBuffer::calcQuadricError(core::matrix4 quadric, CVertex* v, f32 triangleArea) {
	f32 cost = 0;
