...
DecalSceneNode::DecalSceneNode(irr::scene::ISceneNode* parent, scene::ISceneManager* smgr, scene::IMesh* mesh, video::ITexture* texture, const core::vector3df position)
	: irr::scene::ISceneNode(parent, smgr, -1, position, core::vector3df(0, 0, 0), (core::vector3df(1, 1, 1) / parent->getScale())) {
	...
	material.MaterialTypeParam = video::pack_textureBlendFunc(video::EBF_SRC_ALPHA, video::EBF_ONE_MINUS_SRC_ALPHA, video::EMFN_MODULATE_1X, video::EAS_TEXTURE | video::EAS_VERTEX_COLOR);

	material.TextureLayer[0].TextureWrapU = irr::video::ETC_CLAMP_TO_BORDER;
	material.TextureLayer[0].TextureWrapV = irr::video::ETC_CLAMP_TO_BORDER;
	...
	if (fadeOut && lifetime > 0) {
		...
		if (difftime(time(NULL), creationTime) > lifetime) {
			if (startFadeOutTimeMs > 0) {
				...
				const u32 alpha = 255 - ((timeDiff * 255) / (fadeOutTime * 1000));
...
void DecalSceneNode::setFadeOut(const bool fadeOut, const f32 time) {
